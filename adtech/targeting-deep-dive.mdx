...
Targeting is the reason digital advertising is worth $600B+ per year. The ability to show the right ad to the right person at the right time is what separates digital from traditional advertising. It's also the most controversial aspect of ad tech -- touching privacy, regulation, and the fundamental economics of the internet.

<Tip>
**Software engineering analogy**: Targeting is essentially a **recommendation system** at massive scale. Just as Netflix recommends shows and Spotify recommends songs, ad tech recommends ads. The same ML techniques apply: collaborative filtering, content-based filtering, and hybrid approaches. The difference is that ad targeting operates under extreme latency constraints (~10ms), handles billions of daily decisions, and is subject to increasingly strict regulation.
</Tip>

## Types of Targeting

There are two fundamental approaches to targeting ads:
...
Contextual targeting is experiencing a major resurgence because:

1. **No user data needed**: It works without cookies, device IDs, or any personal information
2. **Privacy-compliant by design**: GDPR, CCPA, and iOS ATT don't restrict contextual targeting
3. **Improved technology**: Modern NLP (including LLMs) can understand page context far better than keyword matching could
4. **Brand safety**: Understanding what's *on the page* is essential for ensuring ads don't appear next to inappropriate content

<Accordion title="For the curious: How modern contextual AI works">
Early contextual targeting was simple keyword matching -- the word "basketball" on a page triggered sports ads. This led to problems: an article about a "basketball player's injury" might trigger ads for a sports brand that doesn't want to be associated with injury content.
...
**Behavioral targeting** targets ads based on a user's past behavior -- websites visited, searches performed, products viewed, apps used. It's far more precise than contextual targeting but requires tracking individual users across the internet.
...
graph TD
    ...
    subgraph Targeting["Ad Targeting"]
        T["Nike DSP sees user #abc123<br/>on random news site →<br/>Bids high, shows running shoe ad"]
    end
...
The foundation of behavioral targeting on the web is the **HTTP cookie** -- specifically, **third-party cookies**.
...
<Tabs>
  <Tab title="First-Party Cookies">
    ...
    **Privacy impact**: Low -- the site you're visiting knows you're visiting it
  ...
  <Tab title="Third-Party Cookies">
    **Set by**: A different domain than the one you're visiting (the ad tech domain)
    **Domain**: An ad tech domain (e.g., `doubleclick.net` sets a cookie while you're on `nytimes.com`)
    ...
    **Status**: Being phased out. Blocked by Safari (since 2017), Firefox (since 2019), Chrome (announced deprecation, then reversed in 2024 -- but still adding restrictions)
...
Third-party cookies are just one mechanism for tracking users. The full toolkit includes:
...
<Accordion title="For the curious: How device fingerprinting works">
...
The **canvas fingerprint** is particularly clever: different GPU/driver combinations render the same canvas content slightly differently at the sub-pixel level. By rendering a complex shape and hashing the result, you get a signal that's surprisingly unique.

Fingerprinting is controversial because it's nearly invisible to users and difficult to prevent. Unlike cookies, you can't "clear" a fingerprint. Browsers are fighting back: Safari randomizes canvas rendering, Firefox blocks known fingerprinting scripts, and Chrome is adding anti-fingerprinting measures through the Privacy Sandbox.
</Accordion>
...
A **DMP** is a platform that collects, organizes, and activates audience data for ad targeting. Think of it as the data warehouse of ad tech.
...
<Steps>
  ...
  <Step title="Build Audience Segments">
    ...
    - "Frequent Travelers" -- people who book flights/hotels regularly
    - "High Household Income" -- inferred from browsing/purchase patterns
...
<Tip>
**Software engineering analogy**: A DMP is like a **feature store** in ML infrastructure. It collects raw signals (data), computes features (audience segments), and serves them at low latency for real-time model inference (bid decisions). The same design patterns apply: batch processing for segment computation, real-time serving for lookup, and careful versioning of segment definitions.
</Tip>
...
2. **GDPR/CCPA** limit the collection and sharing of personal data
3. **Data quality concerns**: Third-party segments were often inaccurate and opaque
4. **Rise of CDPs**: Customer Data Platforms (like Segment, mParticle) focus on first-party data, which is more valuable and privacy-compliant

<Note>
The industry is shifting from DMPs (focused on third-party data for targeting) to **CDPs** (focused on first-party data for personalization and activation). This mirrors the broader shift from cookie-based targeting to identity-based and contextual approaches.
</Note>
...
Starting around 2018, a series of regulations and platform changes fundamentally disrupted behavioral targeting:
...
The EU's GDPR, effective May 2018, established:
...
<Warning>
The opt-in rate for ATT is approximately **20-25%**. This means ~75-80% of iOS users chose NOT to allow tracking. For ad tech, this was seismic -- it meant losing cross-app tracking for the majority of the most valuable mobile users (iOS users tend to have higher income and spending).
...
<Steps>
  <Step title="Browser Learns Interests">
    Chrome locally categorizes websites the user visits into ~350 predefined topics (from a public taxonomy). Example topics: "Sports/Basketball," "Technology/Computers," "Travel/Hotels."
  </Step>
...
<Accordion title="For the curious: Topics API technical details">
```javascript
...
- The taxonomy has only ~350 topics -- too coarse for precise individual targeting
...
Mobile app targeting works differently from web targeting:
...
<Note>
Mobile apps have access to far richer data than websites -- device sensors, precise location, in-app behavior, and purchase data. This is why mobile ad targeting has historically been more effective than web targeting. However, ATT on iOS and evolving restrictions on Android are rapidly closing this gap.
</Note>
...
<Tabs>
  <Tab title="Client-Side (Cookie-Based)">
    ...
    **Pros**: Simple, fast
    **Cons**: User can clear cookies, doesn't work cross-device, limited by cookie size
  </Tab>
  ...
  <Tab title="DSP-Level Frequency Capping">
    ...
    - **1-2 per day** for retargeting (don't be creepy)
...
Advertisers don't just target users -- they optimize *how much* they pay for each impression. The purchase algorithm determines the bid strategy.
...
| **CPC** | Cost Per Click | Pay only when user clicks | Traffic / consideration campaigns |
| **CPA** | Cost Per Acquisition | Pay only when user converts (purchase, install, sign-up) | Performance / direct response |
| **ROAS** | Return on Ad Spend | Optimize for revenue generated per dollar spent | E-commerce / revenue optimization |

### How DSP Optimization Works

Modern DSPs use ML models to optimize bids:
...
<Accordion title="For the curious: The bid calculation math">
For a CPA campaign, the bid calculation looks like this:

```
Bid = pCTR × pCVR × Target_CPA × Pacing_Multiplier × Shading_Factor
```
...
```
Bid = 0.005 × 0.02 × $50 × 1.2 × 0.7 = $0.0042 per impression = $4.20 CPM
```
...
**Budget pacing** controls how quickly a campaign spends its budget over time. Without pacing, a popular campaign might exhaust its daily budget by 8am.
...
<Tabs>
  <Tab title="ASAP Pacing">
    ...
    **Risk**: Budget exhausted early in the day. You might miss valuable impressions in the afternoon/evening.
...
<Accordion title="For the curious: Pacing algorithms">
...
2. **Integral**: Has the campaign been consistently over/under pacing? Apply a cumulative correction.
...
The challenge: auction dynamics are non-stationary. Traffic volumes, competition, and user behavior change throughout the day. A static pacing model will oscillate. Production systems use adaptive algorithms that learn these patterns.
</Accordion>
...
2. **Third-party cookies** enabled cross-site tracking but are being phased out through browser changes and regulation.
...
4. **GDPR, CCPA, and iOS ATT** have fundamentally changed what's possible in ad targeting. The industry is adapting with contextual targeting, first-party data, and privacy-preserving APIs like Topics.
