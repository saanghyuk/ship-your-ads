...
Imagine you're a detective solving a case. A customer just bought a pair of running shoes from your app. In the 14 days before that purchase, they saw a YouTube ad, clicked a Facebook ad, tapped a TikTok ad, and searched on Google. **Which ad deserves the credit?** That's the attribution problem -- and it's one of the hardest questions in advertising.

<Tip>
**Software engineering analogy**: Attribution is the **distributed tracing** of advertising. Just as OpenTelemetry traces a request across microservices to understand what happened, attribution traces a user's journey across ad touchpoints to understand what drove a conversion. And just like distributed tracing, it relies on propagating identifiers across system boundaries -- which is exactly what privacy regulations are making harder.
</Tip>
...
<Tabs>
  <Tab title="Click-Through Attribution (CTA)">
    ...
    **Why it's trusted**: The user took an intentional action (clicking). There's a strong signal that the ad influenced behavior.

    **Typical window**: 7-30 days after the click.
  </Tab>
  <Tab title="View-Through Attribution (VTA)">
    The user **saw** an ad (but didn't click) and later converted.
    ...
    **Why it's controversial**: Did the user convert *because* they saw the ad, or would they have converted anyway? It's like a billboard on the highway -- the person may have been driving to that store regardless.
...
When a user interacts with multiple ads before converting, **who gets the credit?** Different models answer this differently.
...
Here's the sneaky part: if you compare Facebook and Google using their own default windows, you're comparing apples to oranges. Always normalize attribution windows when comparing channels side by side.
</Note>
...
This is more sophisticated than heuristic models (linear, time-decay) because it's based on actual observed behavior rather than arbitrary rules.
</Accordion>
...
<Tip>
**MMM is making a comeback** because of privacy changes. When you can't track individual users (iOS ATT, cookie deprecation), aggregate modeling becomes the only way to measure channels like TV, podcasts, and billboards alongside digital. Google's Meridian and Meta's Robyn are open-source MMM tools that have accelerated adoption.
</Tip>

### Incrementality Testing

The gold standard of measurement. Incrementality testing answers: **"What would have happened if we hadn't shown this ad?"**
...
<Warning>
Incrementality tests are expensive -- you're deliberately *not* showing ads to a portion of your audience, which means lost potential conversions during the test period. But they're the only way to prove causation rather than correlation. Think of it as an A/B test for advertising effectiveness itself.
</Warning>
...
Attribution systems need to connect "User A clicked this ad" with "User A installed this app." But how do you know it's the same user?
...
**Accuracy**: ~99%. If the IDFA on the ad click matches the IDFA on the app install, it's definitively the same device.

### Probabilistic Matching

When deterministic IDs aren't available (user opted out, cross-device), attribution falls back to **statistical estimation**.
...
**Accuracy**: ~70-85%. It's an educated guess, not a certainty. Two people on the same office Wi-Fi with the same iPhone model could be confused.
...
**Deterministic device graphs** (Google, Facebook, Amazon) link devices through logged-in user accounts. When you use Gmail on your phone and Chrome on your laptop, Google knows they're you.

**Probabilistic device graphs** (Tapad, LiveRamp, Oracle) use statistical signals to infer device relationships. Less accurate but covers users who aren't logged in.
...
<Steps>
  <Step title="What ATT requires">
    ...
    There's no way for developers to customize this prompt or make it more appealing. Apple controls the exact wording.
  </Step>
  <Step title="Opt-in rates are devastatingly low">
    ...
    - Social media apps: ~15% opt-in
    - Gaming apps: ~20% opt-in
    - Shopping apps: ~25% opt-in
    - Banking/finance apps: ~10% opt-in
  </Step>
...
<Note>
ATT didn't just affect attribution -- it broke the **feedback loop** that ad platforms relied on. Without conversion data flowing back from 75% of iOS users, platforms like Facebook couldn't optimize their ad delivery algorithms effectively. This is why Meta invested $10B+ in AI/ML to rebuild targeting and measurement capabilities without user-level signals.
</Note>
...
Apple created **SKAdNetwork** as a privacy-preserving attribution framework for iOS. It provides attribution without revealing user-level data.
...
<Tabs>
  ...
  <Tab title="SKAN 3.0">
    ...
    Honestly, SKAN 3.0 was a minor upgrade. The fundamental constraints remained.
  </Tab>
  <Tab title="SKAN 4.0 (AdAttributionKit)">
    ...
    | Tier 2    | Medium           | Fine values, 2-dig |
    | Tier 3    | High             | Fine values, 4-dig |
    +-----------+------------------+--------------------+
    ```
...
This lets you report both revenue ranges and key funnel events in a single 6-bit value. MMPs like Airbridge, AppsFlyer, and Adjust provide conversion value management tools that optimize these schemas automatically based on your historical data.
</Accordion>
...
1. Chrome/Android classifies websites and apps into topics from a taxonomy (~470 topics)
...
**Key difference from cookies/IDFA**: Advertisers see coarse interest categories, not specific browsing history or cross-app behavior. You can target "people interested in running" but not "people who viewed Nike Pegasus 41 in size 10."
...
<Tabs>
  <Tab title="Event-Level Reports">
    - Links a specific ad click/view to a conversion
...
<Tip>
**S2S is becoming mandatory, not optional.** Meta now heavily penalizes advertisers who rely solely on the browser pixel. Their algorithm gives significant preference to Conversions API data for optimization. If you're not sending S2S conversions, your campaigns are likely underperforming compared to competitors who do.
</Tip>
...
<CardGroup cols={2}>
  <Card title="Single Source of Truth" icon="database">
    Without an MMP, each ad network reports its own numbers. Facebook says "we drove 1,000 installs," Google says "we drove 800," and TikTok says "we drove 600." But your actual total is only 1,200. Who's right? The MMP deduplicates and provides the real answer.
  </Card>
  ...
  <Card title="Deep Linking" icon="link">
    MMPs route users to specific in-app content -- not just the home screen. This powers personalized onboarding experiences ("Welcome! Here's the product you were looking at") and significantly improves conversion rates.
  </Card>
...
<Tabs>
  <Tab title="AppsFlyer">
    **Founded**: 2011, Israel. **Market position**: Market leader (~65-70% share among top apps).
  ...
  <Tab title="Adjust">
    **Founded**: 2012, Berlin. **Acquired by AppLovin** for $1B in 2021.
    ...
    Strong in the gaming vertical. Being owned by AppLovin (a major ad network) raises neutrality questions, though Adjust operates independently. Known for clean UX, developer-friendly SDK, and strong server-to-server integration capabilities.
  </Tab>
  <Tab title="Branch">
  ...
  <Tab title="Airbridge">
    **Founded**: 2015, South Korea. Growing rapidly in APAC and globally.

    Known for innovative features: unified web+app measurement from day one, incrementality testing capabilities, and strong SKAN support. Has Google and Meta SRN partnerships. Offers flexible pricing that appeals to growth-stage companies. Particularly strong in the Korean and Japanese mobile markets.
  </Tab>
  <Tab title="Singular">
...
<Warning>
**Only ~6 MMPs have certified partnerships with both Google and Meta.** This is the single most important competitive factor in the MMP market. Without these partnerships, an MMP can't receive attribution claims from or send postbacks to Google Ads, YouTube, Facebook, and Instagram -- which account for ~50% of mobile ad spend. This is why the MMP market has consolidated around a handful of players, and why it's extremely difficult for new entrants to compete.
</Warning>
...
SANs don't allow MMP tracking links on their ads. Instead, they report their own attribution claims to the MMP. The MMP must trust the SAN's data and deduplicate across all sources (other networks + all SANs) to determine the final winner.

<Note>
**The SAN advantage in the privacy era**: Because SANs have logged-in user data (your Google account, Facebook profile), they can perform deterministic matching even without IDFA/GAID. This is why Meta and Google's attribution remained relatively strong post-ATT compared to smaller networks that relied entirely on device IDs.
</Note>
...
Postbacks are the server-to-server callbacks that feed conversion data back to ad networks. They're the circulatory system of mobile attribution.
...
**Why postbacks are critical**: They're the feedback loop that makes campaign optimization work. Without timely, accurate postbacks, ad platforms can't train their ML models to find the right users. More postback data = better algorithm optimization = better campaign performance.
...
<Warning>
**Fingerprinting is dead (or should be).** Apple explicitly banned fingerprinting in 2021, stating it violates their developer guidelines regardless of user consent status. Google is moving toward similar restrictions on Android. While some networks still attempt fingerprinting under euphemistic names like "probabilistic matching" or "advanced modeling," it's considered a **violation of platform policies** and can result in app removal from the App Store. Any MMP or network still relying on fingerprinting is playing a dangerous game.
</Warning>
...
The magic of deferred deep linking is that it works **even when the app isn't installed yet**:
...
<Steps>
  ...
  <Step title="App not installed -- redirect to App Store">
    The MMP detects the app isn't installed and redirects the user to the App Store or Play Store.
  </Step>
...
<Tip>
**Why deferred deep links matter for conversion rates**: Without deferred deep linking, a new user clicks an ad for running shoes, installs the app, and lands on the generic home screen. They have to search for the shoes again -- and many won't bother. With deferred deep linking, they land directly on the product page. This can improve conversion rates by **2-3x** because you eliminate the friction of re-finding the product.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="MMPs are essential infrastructure" icon="server">
    Without an MMP, mobile advertisers can't deduplicate conversions, can't get a unified view across channels, and can't send standardized postbacks to optimize campaigns. Only ~6 MMPs have Google + Meta SRN partnerships.
  </Card>
